...
export const CampusesConfiguration = () => {
  ...
  const fetchCampuses = async () => {
    try {
      const {
        data,
        error
      } = await supabase.from('master_campuses').select('*').order('name');
  ...
  const handleSave = async () => {
    try {
      ...
      if (editingCampus) {
        const {
          error
        } = await supabase.from('master_campuses').update(campusData).eq('id', editingCampus.id);
        if (error) throw error;
      } else {
        const {
          error
        } = await supabase.from('master_campuses').insert(campusData);
        if (error) throw error;
      }
      toast({
        title: "Success",
  ...
  const handleDelete = async (campus: MasterCampus) => {
    ...
    try {
      const {
        error
      } = await supabase.from('master_campuses').delete().eq('id', campus.id);
      if (error) throw error;
      toast({
        title: "Success",
  ...
  const filteredCampuses = campuses.filter(campus => campus.name.toLowerCase().includes(searchQuery.toLowerCase()) || campus.city?.toLowerCase().includes(searchQuery.toLowerCase()) || campus.country?.toLowerCase().includes(searchQuery.toLowerCase()));
  ...
  return <div className="space-y-6 px-4 sm:px-6 md:px-[100px] py-4 sm:py-6 md:py-[50px]">
      ...
      {loading ? <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
        ...
        </div> : filteredCampuses.length === 0 ? <Card className="border-dashed">
          ...
          {filteredCampuses.map(campus => <Card key={campus.id} className="group hover:shadow-lg transition-all duration-300 border-2 hover:border-primary/30 overflow-hidden">
              ...
              <CardHeader className="pb-4">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    {campus.code && <Badge variant="secondary" className="mt-2">
                        {campus.code}
                      </Badge>}
                  </div>
                  <div className="flex gap-1">
                    <Button variant="ghost" size="sm" onClick={() => handleEdit(campus)} className="opacity-0 group-hover:opacity-100 transition-opacity">
                      <Edit className="h-4 w-4" />
                    </Button>
                    <Button variant="ghost" size="sm" onClick={() => handleDelete(campus)} className="opacity-0 group-hover:opacity-100 transition-opacity text-destructive hover:text-destructive">
              ...
              <CardContent className="space-y-3">
                {campus.address && <div className="flex items-start gap-2 text-sm">
                    ...
                    <div>
                      ...
                      {campus.postal_code && <p className="text-muted-foreground">{campus.postal_code}</p>}
                    </div>
                  </div>}
                
                <div className="grid grid-cols-2 gap-3 pt-3 border-t">
                  {campus.phone && <div className="flex items-center gap-2 text-sm">
                      <Phone className="h-4 w-4 text-muted-foreground" />
                      <span className="text-foreground truncate">{campus.phone}</span>
                    </div>}
                  {campus.email && <div className="flex items-center gap-2 text-sm">
                      <Mail className="h-4 w-4 text-muted-foreground" />
                      <span className="text-foreground truncate">{campus.email}</span>
                    </div>}
                  {campus.website && <div className="flex items-center gap-2 text-sm col-span-2">
                      <Globe className="h-4 w-4 text-muted-foreground" />
                      <a href={campus.website} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline truncate">
                        {campus.website}
                      </a>
                    </div>}
                </div>
                ...
                <div className="flex items-center justify-between pt-3 border-t">
                  ...
                  {campus.timezone && <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-2 pt-2">
                  {campus.is_active ? <Badge className="bg-green-100 text-green-700 border-green-200 hover:bg-green-100">
            ...
            <div className="flex-1 overflow-y-auto pr-2">
              <TabsContent value="basic" className="space-y-4 mt-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    ...formData,
                    name: e.target.value
                  })} placeholder="Main Campus" className="mt-1.5" />
                  </div>
                  ...
                  <div>
                    ...
                    ...formData,
                    code: e.target.value
                  })} placeholder="MC" className="mt-1.5" />
                  </div>

                  <div className="md:col-span-2">
                    <Label htmlFor="capacity">Student Capacity</Label>
                    <Input id="capacity" type="number" value={formData.capacity || 0} onChange={e => setFormData({
                    ...formData,
                    capacity: parseInt(e.target.value) || 0
                  })} placeholder="5000" className="mt-1.5" />
                  </div>
              ...
              <TabsContent value="location" className="space-y-4 mt-4">
                <div className="grid grid-cols-1 gap-4">
                  ...
                  })} placeholder="123 University Avenue" className="mt-1.5" />
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      ...formData,
                      city: e.target.value
                    })} placeholder="Sydney" className="mt-1.5" />
                    </div>
                    ...
                    })} placeholder="2000" className="mt-1.5" />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    })} placeholder="NSW" className="mt-1.5" />
                    </div>
                    ...
                    })} placeholder="Australia" className="mt-1.5" />
                    </div>
                  </div>

                  <div>
                    <Label htmlFor="timezone">Timezone</Label>
                    <Input id="timezone" value={formData.timezone || 'UTC'} onChange={e => setFormData({
                    ...formData,
                    timezone: e.target.value
                  })} placeholder="Australia/Sydney" className="mt-1.5" />
              ...
              <TabsContent value="contact" className="space-y-4 mt-4">
                <div className="grid grid-cols-1 gap-4">
                  ...
                  })} placeholder="+61 2 1234 5678" className="mt-1.5" />
                  </div>
                  ...
                  })} placeholder="campus@university.edu" className="mt-1.5" />
                  </div>
                  ...
                  })} placeholder="https://campus.university.edu" className="mt-1.5" />
                  </div>
                </div>
              </TabsContent>
            </div>
