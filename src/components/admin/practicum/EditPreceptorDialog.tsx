...
import { useUpdatePreceptor } from '@/hooks/usePreceptors';
...
interface EditPreceptorDialogProps {
  open: boolean;
...
export function EditPreceptorDialog({ 
  open, 
  onOpenChange, 
  preceptor,
  siteName 
}: EditPreceptorDialogProps) {
  const updatePreceptor = useUpdatePreceptor();
  ...
  // Reset form when preceptor changes
  useEffect(() => {
    if (preceptor) {
      form.reset({
        ...
        specialization: preceptor.specialization || '',
        years_experience: preceptor.years_experience || undefined,
        license_number: preceptor.license_number || '',
        license_expiry: preceptor.license_expiry ? new Date(preceptor.license_expiry) : undefined,
        is_primary_contact: preceptor.is_primary_contact || false,
        is_active: preceptor.is_active ?? true,
        max_students: preceptor.max_students || 3,
        availability_notes: preceptor.availability_notes || '',
        emergency_contact_name: preceptor.emergency_contact_name || '',
        emergency_contact_phone: preceptor.emergency_contact_phone || '',
        bio: preceptor.bio || '',
      });
    }
  }, [preceptor, form]);
  ...
  const onSubmit = async (data: PreceptorFormData) => {
    ...
    try {
      await updatePreceptor.mutateAsync({
        id: preceptor.id,
        ...data,
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Edit Preceptor at {siteName}</DialogTitle>
        </DialogHeader>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="flex justify-end gap-3">
              ...
              <Button type="submit" disabled={updatePreceptor.isPending}>
                {updatePreceptor.isPending ? 'Updating...' : 'Update Preceptor'}
              </Button>
            </div>
          </form>
        </Form>
