  ...
  return (
    <div className="flex min-h-screen bg-gray-100 w-full">
      ...
      <aside className={`${isCollapsed ? 'w-16' : 'w-72'} bg-gradient-to-b from-background via-background/95 to-muted/30 border-r border-border/50 shadow-lg backdrop-blur-sm fixed inset-y-0 left-0 z-50 h-screen transition-all duration-300 flex flex-col overflow-hidden`}>
        ...
        {isCollapsed && (
          <div className="absolute top-4 -right-3 z-10">
            <Button
              ...
              size="sm"
              onClick={() => setIsCollapsed(!isCollapsed)}
              className="p-1 h-6 w-6 bg-background border border-border shadow-sm rounded-full"
            >
        ...
        {isCollapsed && (
          <div className="p-3 border-b border-gray-200 flex flex-col items-center space-y-4">
            <div 
              className="w-10 h-10 rounded-full bg-primary flex items-center justify-center text-primary-foreground font-medium cursor-pointer hover:bg-primary/90 transition-colors"
              onClick={() => setIsProfileOpen(true)}
              title="View Profile"
            >
                    ...
                    if (isCollapsed) {
                      return (
                        <Link
                          ...
                          className={`group flex items-center justify-center w-10 h-10 mx-auto mb-2 rounded-xl transition-all duration-300 ${
                            isActive 
                        ...
                        >
                          ...
                          {(item.hasUpdate || item.isHighlighted) && !isActive && (
