  ...
  return <div className="flex min-h-screen bg-gray-100 w-full">
      ...
      <aside className={`${isCollapsed ? 'w-16' : 'w-72'} bg-gradient-to-b from-background via-background/95 to-muted/30 border-r border-border/50 shadow-lg backdrop-blur-sm fixed inset-y-0 left-0 z-50 h-screen transition-all duration-300 flex flex-col overflow-hidden`}>
        ...
        <div className="p-4 flex items-center justify-between border-b border-gray-200">
          {!isCollapsed ? <img src="/wcc-logo.png" alt="Western Community College" className="h-14" /> : <div className="flex justify-center w-full">
              <img src="/wcc-logo.png" alt="WCC" className="h-8 w-8 object-contain" />
            </div>}
          {!isCollapsed && <Button variant="ghost" size="sm" onClick={() => setIsCollapsed(!isCollapsed)} className="p-1 h-8 w-8 flex-shrink-0">
              <ChevronLeft size={16} />
            </Button>}
        </div>
        ...
        <nav className={`flex-1 ${isCollapsed ? 'px-2' : 'px-3'} py-4`}>
          <div className="space-y-6">
            {navSections.map(section => <div key={section.id} className="space-y-2">
                ...
                if (isCollapsed) {
                  return <Link key={item.name} to={item.path} title={`${item.name}: ${item.description}`} className={`group flex items-center justify-center w-10 h-10 mx-auto mb-2 rounded-xl transition-all duration-300 ${isActive ? 'bg-primary text-primary-foreground shadow-lg ring-2 ring-primary/20' : 'text-muted-foreground hover:bg-muted hover:text-foreground hover:shadow-sm'} ${item.isHighlighted ? 'ring-2 ring-primary/30 bg-primary/5' : ''}`}>
                          <item.icon className={`h-5 w-5 transition-colors duration-300 ${isActive ? 'text-primary-foreground' : item.isHighlighted ? 'text-primary' : 'text-muted-foreground group-hover:text-foreground'}`} />
                          
                          {/* Notification dots for collapsed view */}
                          {(item.hasUpdate || item.isHighlighted) && !isActive && <div className="absolute top-1 right-1 w-2 h-2 bg-primary rounded-full animate-pulse" />}
                        </Link>;
                }
                return <Link key={item.name} to={item.path} className={`group flex items-center gap-3 px-3 py-3 rounded-xl transition-all duration-300 relative overflow-hidden ${isActive ? 'bg-gradient-to-r from-primary/90 to-primary text-primary-foreground shadow-lg transform scale-[1.02] ring-2 ring-primary/20' : 'text-muted-foreground hover:bg-muted/80 hover:text-foreground hover:transform hover:scale-[1.01] hover:shadow-sm'} ${item.isHighlighted ? 'ring-2 ring-primary/30 bg-primary/5' : ''}`}>
                        {/* Highlight glow for important items */}
                        {item.isHighlighted && !isActive && <div className="absolute inset-0 bg-gradient-to-r from-primary/10 to-transparent opacity-50 rounded-xl" />}
                        
                        <div className={`p-2 rounded-lg transition-colors duration-300 ${isActive ? 'bg-white/20' : item.isHighlighted ? 'bg-primary/10' : 'bg-muted/50 group-hover:bg-muted'}`}>
                          <item.icon className={`h-4 w-4 flex-shrink-0 transition-colors duration-300 ${isActive ? 'text-primary-foreground' : item.isHighlighted ? 'text-primary' : 'text-muted-foreground group-hover:text-foreground'}`} />
                        </div>
                        
                        {!isCollapsed && <div className="flex-1 min-w-0">
                            <div className={`font-medium text-sm transition-colors duration-300 ${isActive ? 'text-primary-foreground' : item.isHighlighted ? 'text-primary' : 'text-foreground group-hover:text-foreground'}`}>
                              {item.name}
                            </div>
                            <div className={`text-xs leading-tight truncate transition-colors duration-300 ${isActive ? 'text-primary-foreground/80' : 'text-muted-foreground group-hover:text-muted-foreground'}`}>
                              {item.description}
                            </div>
                          </div>}
                        
                        {!isCollapsed && item.hasUpdate && <div className="w-2 h-2 bg-orange-500 rounded-full animate-pulse" />}
                        
                        {item.isHighlighted && !isCollapsed && !isActive && <div className="w-2 h-2 bg-primary rounded-full animate-pulse" />}
                      </Link>;
              })}
                </div>
              </div>)}
        ...
        {!isCollapsed && <div className="mt-auto p-4 border-t border-gray-200 space-y-3">
            <a href="#" className="flex items-center justify-center gap-2 px-4 py-2 border border-primary rounded-md text-primary hover:bg-primary-light text-sm" onClick={e => e.preventDefault()}>
