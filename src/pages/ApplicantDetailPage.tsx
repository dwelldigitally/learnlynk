...
const ApplicantDetailPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        setApplicant(data);
        document.title = data?.master_records ? `${data.master_records.first_name} ${data.master_records.last_name} — Applicant` : "Applicant — Detail";
      } catch (e) {
        console.error(e);
        toast({
          title: "Error",
          description: "Failed to load applicant",
          variant: "destructive"
        });
      } finally {
        setLoading(false);
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      toast({
        title: "Approved",
        description: "Applicant moved to student stage"
      });
      ...
      toast({
        title: "Error",
        description: "Could not approve applicant",
        variant: "destructive"
      });
    } finally {
      setSaving(false);
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: "Could not reject applicant",
        variant: "destructive"
      });
    } finally {
      setSaving(false);
  ...
  const handleCreateSample = async () => {
    try {
      ...
      toast({
        title: "Error",
        description: "Could not create sample applicant",
        variant: "destructive"
      });
    } finally {
      setSaving(false);
  ...
  const handleApproveAllDocs = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "All submitted docs marked approved"
      });
    } catch (e) {
      toast({
        title: "Error",
        description: "Could not update documents",
        variant: "destructive"
      });
    } finally {
      setSaving(false);
  ...
  const applicantName = applicant.master_records ? `${applicant.master_records.first_name} ${applicant.master_records.last_name}` : "Applicant";
  ...
  return <ModernAdminLayout>
      <div className="min-h-screen bg-gradient-to-br from-background via-background to-muted/20">
        <div className="p-6 lg:p-8 space-y-6">
          ...
          {aiViewMode ? <div className="space-y-6">
              <div className="bg-card/80 backdrop-blur-sm border border-border/50 rounded-xl p-6 shadow-lg">
                <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                  ...
                  <div className="flex items-center gap-3">
                    <Button onClick={() => navigate(`/admin/applicants/review/${applicantId}`)} className="flex items-center gap-2 shadow-md hover:shadow-lg transition-shadow">
                      <ClipboardCheck className="h-4 w-4" />
                      Enter Review Mode
                    </Button>
                    <Button variant="outline" onClick={() => setAiViewMode(false)} className="flex items-center gap-2 border-border/50 hover:bg-muted/50 transition-all">
            ...
            <EnhancedApplicantHeader applicant={applicant} onApprove={handleApprove} onReject={handleReject} onScheduleInterview={handleScheduleInterview} onSendEmail={handleSendEmail} saving={saving} />
          </div> : <div className="space-y-6">
              
            <ApplicantHeader applicant={applicant} onApprove={handleApprove} onReject={handleReject} onScheduleInterview={handleScheduleInterview} onSendEmail={handleSendEmail} onSwitchToAiMode={() => setAiViewMode(true)} saving={saving} />
            </div>}
                  ...
                  <Tabs value={activeTab} onValueChange={setActiveTab}>
                    ...
                    <div className="p-6">
                      ...
                      <TabsContent value="documents" className="space-y-6 m-0">
                        <SmartDocumentReview applicant={applicant} onDocumentAction={handleDocumentAction} onApproveAll={handleApproveAllDocs} />
                      </TabsContent>

                      <TabsContent value="assessment" className="space-y-6 m-0">
                        <AssessmentPanel applicant={applicant} onScoreUpdate={handleScoreUpdate} onNotesUpdate={handleAssessmentNotesUpdate} />
                      </TabsContent>

                      <TabsContent value="timeline" className="space-y-6 m-0">
                        <ApplicationTimeline applicant={applicant} onStageUpdate={handleStageUpdate} />
                      </TabsContent>
                    </div>
                  </Tabs>
                </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-4 gap-6">
                  ...
                  <Tabs value={activeTab} onValueChange={setActiveTab}>
                    ...
                    <div className="p-6">
                      ...
                      <TabsContent value="documents" className="m-0">
                        <DocumentManager applicant={applicant} onDocumentAction={handleDocumentAction} onApproveAll={handleApproveAllDocs} />
                      </TabsContent>

                      <TabsContent value="communication" className="m-0">
                        <CommunicationCenter applicantId={applicant.id} applicantName={applicantName} applicantEmail={applicant.master_records?.email || ''} onSendMessage={handleSendMessage} />
                      </TabsContent>
              ...
              <div className="xl:col-span-1">
                <div className="animate-fade-in">
                  <CollaborationSidebar applicantId={applicant.id} onAssign={handleAssign} onComment={handleComment} />
                </div>
              </div>
            </div>}
        </div>
