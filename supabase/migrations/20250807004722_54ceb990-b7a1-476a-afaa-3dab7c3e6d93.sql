...
USING (
  EXISTS (
    SELECT 1 FROM public.recruiter_users ru 
    WHERE ru.id = recruiter_applications.recruiter_id 
...
USING (
  EXISTS (
    SELECT 1 FROM public.recruiter_applications ra
    JOIN public.recruiter_users ru ON ra.recruiter_id = ru.id
    WHERE ra.id = recruiter_communications.recruiter_application_id
    AND ru.user_id = auth.uid()
  )
...
USING (
  EXISTS (
    SELECT 1 FROM public.recruiter_applications ra
    JOIN public.recruiter_users ru ON ra.recruiter_id = ru.id
    WHERE ra.id = recruiter_documents.recruiter_application_id
    AND ru.user_id = auth.uid()
  )
  OR auth.role() = 'authenticated'
);
...
WITH CHECK (
  EXISTS (
    SELECT 1 FROM public.recruiter_users ru 
    WHERE ru.id = recruiter_documents.uploaded_by 
    AND ru.user_id = auth.uid()
  )
);
