...
-- Insert default substages for each stage
INSERT INTO public.stage_substages (user_id, stage, substage_key, substage_name, substage_description, order_index) VALUES
-- Lead substages
(auth.uid(), 'lead', 'new_inquiry', 'New Inquiry', 'Initial contact or inquiry received', 1),
(auth.uid(), 'lead', 'qualification', 'Qualification', 'Lead is being qualified for fit', 2),
(auth.uid(), 'lead', 'nurturing', 'Nurturing', 'Lead is being nurtured with content and follow-ups', 3),
(auth.uid(), 'lead', 'ready_to_apply', 'Ready to Apply', 'Lead is ready to start application process', 4),

-- Applicant substages
(auth.uid(), 'applicant', 'application_started', 'Application Started', 'Application process has begun', 1),
(auth.uid(), 'applicant', 'documents_submitted', 'Documents Submitted', 'Required documents have been submitted', 2),
(auth.uid(), 'applicant', 'under_review', 'Under Review', 'Application is being reviewed by admissions', 3),
(auth.uid(), 'applicant', 'decision_pending', 'Decision Pending', 'Waiting for final admission decision', 4),

-- Student substages  
(auth.uid(), 'student', 'enrolled', 'Enrolled', 'Student has been enrolled and confirmed', 1),
(auth.uid(), 'student', 'orientation', 'Orientation', 'Student is attending orientation programs', 2),
(auth.uid(), 'student', 'active', 'Active', 'Student is actively attending classes', 3),
(auth.uid(), 'student', 'graduated', 'Graduated', 'Student has successfully graduated', 4),
(auth.uid(), 'student', 'alumni', 'Alumni', 'Former student, now alumnus', 5);