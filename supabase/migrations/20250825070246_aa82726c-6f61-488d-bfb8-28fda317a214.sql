-- Create dummy data for enrollment optimization with correct table structures
DO $$
DECLARE
    demo_user_id uuid;
BEGIN
    ...
    -- Insert dummy data for action_queue (already correct structure)
    ...
    -- Insert dummy data for signals (using correct columns)
    INSERT INTO signals (
      user_id, webinar_attended, form_submitted_at, last_email_open_at, 
      pageviews_7d, yield_score, intent_signals, created_at
    ) VALUES 
    (demo_user_id, true, NOW() - INTERVAL '1 day', NOW() - INTERVAL '2 hours', 15, 85.5, '{"high_engagement": true, "multiple_touchpoints": true}', NOW()),
    (demo_user_id, true, NOW() - INTERVAL '2 days', NOW() - INTERVAL '1 day', 8, 78.2, '{"campus_visit": true, "scholarship_interest": true}', NOW()),
    (demo_user_id, false, NOW() - INTERVAL '3 days', NOW() - INTERVAL '3 hours', 23, 72.1, '{"webinar_registered": true, "email_engagement": "high"}', NOW()),
    (demo_user_id, false, NOW() - INTERVAL '1 week', NOW() - INTERVAL '1 day', 5, 65.4, '{"social_media_engagement": true}', NOW()),
    (demo_user_id, true, NOW() - INTERVAL '1 hour', NOW() - INTERVAL '30 minutes', 32, 91.8, '{"multiple_applications": true, "urgent_interest": true}', NOW());

    -- Insert dummy data for waste_radar (using correct columns)
    INSERT INTO waste_radar (
      user_id, student_name, unresponsive_30d, wrong_intake, duplicate_flag,
      last_meaningful_contact, touch_count, created_at
    ) VALUES 
    (demo_user_id, 'Robert Garcia', true, false, false, NOW() - INTERVAL '30 days', 8, NOW()),
    (demo_user_id, 'Test Duplicate', false, false, true, NOW() - INTERVAL '14 days', 5, NOW()),
    (demo_user_id, 'Inactive Student', true, true, false, NOW() - INTERVAL '60 days', 12, NOW()),
    (demo_user_id, 'Over Contacted', false, false, false, NOW() - INTERVAL '2 days', 25, NOW());

    -- Insert dummy data for policy_configurations (using correct columns)
    ...
    -- Insert dummy data for outcome_metrics (using correct columns)
