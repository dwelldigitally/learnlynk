...
DECLARE
    demo_user_id uuid;
    student_uuid1 uuid := gen_random_uuid();
    student_uuid2 uuid := gen_random_uuid();
    student_uuid3 uuid := gen_random_uuid();
    student_uuid4 uuid := gen_random_uuid();
    student_uuid5 uuid := gen_random_uuid();
BEGIN
    ...
    -- Insert dummy data for action_queue
    ...
    (demo_user_id, student_uuid1, 'Sarah Johnson', 'Computer Science', 92, 'high', '["high_engagement", "multiple_touchpoints"]', 'Schedule immediate call', NOW() + INTERVAL '2 hours', 'pending', 'urgent', NOW()),
    (demo_user_id, student_uuid2, 'Michael Chen', 'Business Administration', 87, 'high', '["campus_visit", "scholarship_interest"]', 'Send scholarship information', NOW() + INTERVAL '4 hours', 'pending', 'high', NOW()),
    (demo_user_id, student_uuid3, 'Emily Rodriguez', 'Engineering', 76, 'medium', '["webinar_attendance", "email_opens"]', 'Follow-up email with program details', NOW() + INTERVAL '6 hours', 'pending', 'medium', NOW()),
    (demo_user_id, student_uuid4, 'David Thompson', 'Marketing', 68, 'medium', '["social_media_engagement"]', 'Connect on LinkedIn', NOW() + INTERVAL '8 hours', 'pending', 'medium', NOW()),
    (demo_user_id, student_uuid5, 'Jessica Lee', 'Data Science', 84, 'high', '["multiple_applications", "high_grades"]', 'Priority admission review', NOW() + INTERVAL '3 hours', 'pending', 'high', NOW());

    -- Insert dummy data for signals with student_id
    INSERT INTO signals (
    ...
    (demo_user_id, student_uuid1, true, NOW() - INTERVAL '1 day', NOW() - INTERVAL '2 hours', 15, 85.5, '{"high_engagement": true, "multiple_touchpoints": true}', NOW()),
    (demo_user_id, student_uuid2, true, NOW() - INTERVAL '2 days', NOW() - INTERVAL '1 day', 8, 78.2, '{"campus_visit": true, "scholarship_interest": true}', NOW()),
    (demo_user_id, student_uuid3, false, NOW() - INTERVAL '3 days', NOW() - INTERVAL '3 hours', 23, 72.1, '{"webinar_registered": true, "email_engagement": "high"}', NOW()),
    (demo_user_id, student_uuid4, false, NOW() - INTERVAL '1 week', NOW() - INTERVAL '1 day', 5, 65.4, '{"social_media_engagement": true}', NOW()),
    (demo_user_id, student_uuid5, true, NOW() - INTERVAL '1 hour', NOW() - INTERVAL '30 minutes', 32, 91.8, '{"multiple_applications": true, "urgent_interest": true}', NOW());

    -- Insert dummy data for waste_radar with student_id
    ...
    (demo_user_id, gen_random_uuid(), 'Robert Garcia', true, false, false, NOW() - INTERVAL '30 days', 8, NOW()),
