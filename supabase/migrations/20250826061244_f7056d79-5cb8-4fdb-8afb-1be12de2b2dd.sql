-- Insert demo student actions with comprehensive metadata (simplified version)
...
('770e8400-e29b-41d4-a716-446655440001', auth.uid(), 'student_001', 'call', 'Call Sarah Johnson immediately - new MBA inquiry', ARRAY['Speed-to-Lead', 'High yield', 'New inquiry'], 1, 'pending', now() - interval '30 minutes', now(), jsonb_build_object(
...
('770e8400-e29b-41d4-a716-446655440002', auth.uid(), 'student_002', 'email', 'Follow up with Michael Chen on stalled application', ARRAY['Application Stalled', 'High potential', '7-day stall'], 1, 'pending', now() - interval '2 hours', now(), jsonb_build_object(
...
('770e8400-e29b-41d4-a716-446655440003', auth.uid(), 'student_003', 'call', 'Contact Emma Rodriguez about deposit decision', ARRAY['Deposit Follow-up', 'Decision pending', 'High value'], 1, 'pending', now() + interval '1 hour', now(), jsonb_build_object(
...
-- Calls to Make
('770e8400-e29b-41d4-a716-446655440004', auth.uid(), 'student_004', 'call', 'Schedule interview with David Kim for MBA program', ARRAY['Interview Scheduler', 'Qualified candidate'], 2, 'pending', now() + interval '2 hours', now(), jsonb_build_object(
...
('770e8400-e29b-41d4-a716-446655440005', auth.uid(), 'student_005', 'call', 'Call Amanda Thompson about program details', ARRAY['Speed-to-Lead', 'Information request'], 2, 'pending', now() + interval '3 hours', now(), jsonb_build_object(
...
('770e8400-e29b-41d4-a716-446655440006', auth.uid(), 'student_006', 'email', 'Send follow-up email to Robert Wilson about application status', ARRAY['Application Nurture', '3-day follow-up'], 2, 'pending', now() + interval '4 hours', now(), jsonb_build_object(
...
('770e8400-e29b-41d4-a716-446655440007', auth.uid(), 'student_007', 'email', 'Send program brochure to Lisa Garcia', ARRAY['Manual follow-up', 'Information request'], 3, 'pending', now() + interval '1 day', now(), jsonb_build_object(
...
('770e8400-e29b-41d4-a716-446655440008', auth.uid(), 'student_008', 'sms', 'Send SMS reminder to Jessica White about interview', ARRAY['Manual SMS', 'Interview reminder'], 2, 'pending', now() + interval '5 hours', now(), jsonb_build_object(
...
-- Document Reviews
('770e8400-e29b-41d4-a716-446655440009', auth.uid(), 'student_009', 'document_request', 'Request transcripts from Jennifer Lopez', ARRAY['Document Collection', 'Missing transcripts'], 2, 'pending', now() + interval '6 hours', now(), jsonb_build_object(
...
('770e8400-e29b-41d4-a716-446655440010', auth.uid(), 'student_010', 'review', 'Review application documents for Daniel Taylor', ARRAY['Document Collection', 'Review needed'], 3, 'pending', now() + interval '8 hours', now(), jsonb_build_object(
...
('770e8400-e29b-41d4-a716-446655440011', auth.uid(), 'student_011', 'call', 'OVERDUE: Call Marcus Johnson about enrollment deadline', ARRAY['Speed-to-Lead', 'Deadline approaching', 'URGENT'], 1, 'pending', now() - interval '1 day', now(), jsonb_build_object(
...
('770e8400-e29b-41d4-a716-446655440012', auth.uid(), 'student_012', 'email', 'OVERDUE: Request missing documents from Tony Martinez', ARRAY['Document Collection', '5 days overdue'], 1, 'pending', now() - interval '5 days', now(), jsonb_build_object(
...
('770e8400-e29b-41d4-a716-446655440013', auth.uid(), 'student_013', 'call', 'Process deposit for Rachel Green - ready to enroll!', ARRAY['Deposit Follow-up', 'Ready to enroll', 'High value'], 1, 'pending', now() + interval '30 minutes', now(), jsonb_build_object(
...
('770e8400-e29b-41d4-a716-446655440014', auth.uid(), 'student_014', 'email', 'Follow up with Kevin Brown on decision timeline', ARRAY['Application Nurture', 'Decision pending', 'High potential'], 2, 'pending', now() + interval '2 days', now(), jsonb_build_object(
